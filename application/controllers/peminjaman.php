  ...
  public function tambah_pinjam($id){         
  	if($this->session->userdata('status') != "login"){             
		  ...
		  public function lihat_keranjang(){         
			  $data['anggota'] = $this->M_perpus->edit_data(array('id_anggota'=> $this->session->userdata('id_anggota')),'anggota')->result();         
			  ...
			  $d=$this->M_perpus->edit_data(array('id_anggota' => $this->session>userdata('id_anggota')),'transaksi')->num_rows();         
			  if($d>0){             
